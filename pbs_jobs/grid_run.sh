...
for layer in 40 ; do                   
    for lr in  0.001 ; do
        ...
        # for dropout in  0.55  0.75  0.85  0.95  1.0; do
            echo "Layer size: $layer   Task LR: $task_lr  Dropout: $dropout      Config: $config"
            qsub pbs_train.sh -v epochs=${epochs},layer=${layer},dropout=${dropout},task_lr=${lr},outdir=${outdir} 
        done
    done
done